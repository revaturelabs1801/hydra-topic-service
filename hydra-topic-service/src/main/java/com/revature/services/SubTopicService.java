...
public class SubTopicService {
	...
	public SubTopicService(RestTemplate restTemplate, SubtopicRepository subtopicRepository,
			SubtopicNameRepository subtopicNameRepository, SubtopicStatusRepository subtopicStatusRepository,
			SubtopicTypeRepository subtopicTypeRepository) {
		super();
		this.restTemplate = restTemplate;
		this.subtopicRepository = subtopicRepository;
		this.subtopicNameRepository = subtopicNameRepository;
		this.subtopicStatusRepository = subtopicStatusRepository;
		this.subtopicTypeRepository = subtopicTypeRepository;
	}
	 ...
	 * 
	 * @author Jordan DeLong, Cristian Hermida, Charlie Harris / Batch
	...
	/**
	 * Maps curriculum subtopics into subtopics. Each subtopic date is determined by
	 * the start date of the batch offset by the week and day of the corresponding
	 * curriculum subtopic.
	 * 
	 * @author Jordan DeLong, Cristian Hermida, Charlie Harris / Batch
	 *         1712-dec10-java-steve
	...
	public List<Subtopic> mapCurriculumSubtopicsToSubtopics(Map<Integer, List<CurriculumSubtopic>> map, int batchid) {
		...
		// spin up multiple threads that concurrently instantiate new subtopics from
		// lists of curriculum subtopics and add each new subtopic instance to a list of
		// subtopics that will be persisted once all threads have finished
		map.forEach((day, weeks) -> {
			...
			for (CurriculumSubtopic curriculumSubtopic : weeks) {
				...
				// a random starting hour between 9:00 am and 4:00 pm is assigned for each new
				// subtopic
				// +5 hours are added to the min and max of range to compensate for production
				// server timezone difference
				int randomNum = rand.nextInt((21 - 14) + 1) + 14; // nextInt is normally exclusive of the top value, so
				...
				Batch b = restTemplate.getForObject("http://HYDRA-BATCH-SERVICE/api/v2/Batch/byid/" + batchid,
						Batch.class);
				...
				// determine what the actual date on the calendar will be by adding the offset
				// to the currently set calendar day (the batch start date)
				...
				// add the subtopic to a list of subtopics that will be persisted to the
				// database
